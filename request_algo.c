...
int m,n,total[MAX],avail[MAX],alloc[MAX][MAX],max[MAX][MAX],need[MAX][MAX],work[MAX],finish[MAX],seq[MAX],request[MAX];
...
int main()
{
 ...
 if(j==m)
 {
  ...
  if(j==m)
  {
   ...
   printf("Process P%d can be granted.\n",pno);
 }
 else
  printf("Process P%d can't be granted\n",pno);
